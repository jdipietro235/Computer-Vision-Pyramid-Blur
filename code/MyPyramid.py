...
def main():
    ...
    '''
    aBlue, aGreen, aRed = cv2.split(imageA)
    bBlue, bGreen, bRed = cv2.split(imageB)

    print type(aBlue)
    print aBlue.shape


    aBlue = aBlue[..., np.newaxis]
    aGreen = aGreen[..., np.newaxis]
    aRed = aRed[..., np.newaxis]

    bBlue = bBlue[..., np.newaxis]
    bGreen = bGreen[..., np.newaxis]
    bRed = bRed[..., np.newaxis]


    
    '''
    laplA = laplA[..., np.newaxis]
    aBlue = np.asarray(aBlue)#, dtype=np.float32)
    aGreen = np.asarray(aGreen, dtype=np.float32)
    aRed = np.asarray(aRed, dtype=np.float32)

    bBlue = np.asarray(bBlue, dtype=np.float32)
    bGreen = np.asarray(bGreen, dtype=np.float32)
    bRed = np.asarray(bRed, dtype=np.float32)
    '''
    pyrAblue = eacher(aBlue, kernel)
    pyrAgreen = eacher(aGreen, kernel)
    pyrAred = eacher(aRed, kernel)

    pyrBblue = eacher(bBlue, kernel)
    pyrBgreen = eacher(bGreen, kernel)
    pyrBred = eacher(bRed, kernel)

    joinedPyrBlue = halves(pyrAblue, pyrBblue)
    joinedPyrGreen = halves(pyrAgreen, pyrBgreen)
    joinedPyrRed = halves(pyrAred, pyrBred)

    collapsedBlue = collapse(joinedPyrBlue, kernel)
    collapsedGreen = collapse(joinedPyrGreen, kernel)
    collapsedRed = collapse(joinedPyrRed, kernel)

    stackedA = collapsedBlue, collapsedGreen, collapsedRed
...
def eacher(image, kernel):
    print('eacher start')
    [gPyr, lPyr] = pyramids(image, kernel)
